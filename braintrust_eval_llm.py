...
# Import for direct Anthropic API integration
import openai
...
# Setup AutoEvals using company's LiteLLM proxy
def setup_autoevals_with_anthropic():
    ...
    try:
        ...
        print("✅ AutoEvals RAGAS configured with company's LiteLLM proxy")
        ...
        print(f"⚠️ AutoEvals RAGAS setup failed: {e}")
...
# =============================================================================
# AutoEvals RAGAS Scorers using LiteLLM endpoint
# ===============================================

def autoevals_context_relevancy_scorer(input, output, expected=None, context=None):
    """AutoEvals RAGAS ContextRelevancy scorer using Anthropic API"""
    if not autoevals_ready:
        print("⚠️ AutoEvals not ready, returning fallback score")
        return 0.5

    try:
        # For RAGAS, we need context - try context map first, then current_context, then retrieval
        global current_context, context_map
        if not context and input in context_map:
            context = context_map[input]
            print(f"🔍 Using context from context_map for question: {input[:50]}...")
        elif not context and current_context:
            context = current_context
        elif not context:
            context = "Retrieved context not available"  # Default fallback
        ...
        # Use ContextRelevancy with configured Anthropic model
        relevancy = ContextRelevancy(model=env('LITELLM_MODEL'))
        ...
        return Score(
            name="autoevals_context_relevancy",
            score=result.score,
            metadata={
                "rationale": result.metadata.get("rationale", ""),
                "evaluator": "AutoEvals RAGAS ContextRelevancy",
        ...
        print(f"AutoEvals ContextRelevancy failed: {e}")
        from autoevals import Score
        return Score(
            name="autoevals_context_relevancy",
            score=0.5,
...
def autoevals_faithfulness_scorer(input, output, expected=None, context=None):
    """AutoEvals RAGAS Faithfulness scorer using Anthropic API"""
    if not autoevals_ready:
        print("⚠️ AutoEvals not ready, returning fallback score")
        return 0.5
    ...
    try:
        ...
        # Use Faithfulness with configured Anthropic model
        faithfulness = Faithfulness(model=env('LITELLM_MODEL'))
        ...
        from autoevals import Score
        return Score(
            name="autoevals_faithfulness",
            score=result.score,
            metadata={
                "rationale": result.metadata.get("rationale", ""),
                "evaluator": "AutoEvals RAGAS Faithfulness",
                "model": env('LITELLM_MODEL'),
                "via": "litellm_proxy"
            }
        )
        ...
        print(f"AutoEvals Faithfulness failed: {e}")
        import traceback
        traceback.print_exc()
        from autoevals import Score
        return Score(
...
def run_rag_evaluation(dataset_path: str = None, limit: int = None, use_llm_judges: bool = True):
    ...
    if use_llm_judges:
        ...
        scorers = [
            ...
            # AutoEvals RAGAS scorers using LiteLLM endpoint
            autoevals_context_relevancy_scorer,  # RAGAS context relevancy
            autoevals_faithfulness_scorer,       # RAGAS faithfulness
            # Include Non-LLM scorers for comparison
            ExactMatch(),
            Levenshtein()
        ]
        scorer_description = "Custom LiteLLM + AutoEvals RAGAS + Non-LLM"
        evaluation_method = "hybrid_custom_ragas_scorers"
    else:
        scorers = [
            ExactMatch(),
            Levenshtein(),
    ...
    if use_llm_judges:
        ...
        if autoevals_ready:
            print("   🔧 AutoEvals RAGAS (via LiteLLM endpoint):")
            print("     • Context Relevancy (RAGAS)")
            print("     • Faithfulness (RAGAS)")
        else:
            print("   ⚠️ AutoEvals RAGAS: Not available (initialization failed)")
        print("   📈 Comparison: Custom vs AutoEvals RAGAS performance")
    ...
    try:
        ...
        result = Eval(
            ...
            task=rag_task_with_context,  # Task function that handles context
